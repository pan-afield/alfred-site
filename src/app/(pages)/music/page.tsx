    ...
    <div className="max-w-6xl mx-auto px-6">
      <div className="flex justify-between items-end mb-16">
        <div>
          ...
          <p className="text-text-dim font-mono text-sm tracking-tighter uppercase italic">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {musicList.map((music: any) => (
          <div key={music._id} className="group relative">
            {/* 黑胶盘装饰效果 (Hover时滑出) */}
            <div
              className="absolute top-4 right-0 w-48 h-48 bg-zinc-800 rounded-full border-4 border-zinc-700 shadow-2xl transition-transform duration-700 group-hover:translate-x-12 hidden md:block"
              style={{
                backgroundImage:
                  'repeating-radial-gradient(circle, #222 0%, #111 2%, #222 4%)',
              }}
            >
