...
export const CinemaCard = ({
  ...
  return (
    <BaseCard
      className="md:col-span-2 md:row-span-1 overflow-hidden p-0 group"
      path={path}
      ariaLabel={ariaLabel}
    >
      ...
      <div className="relative h-full w-full p-5 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-[9px] text-text-dim tracking-[0.2em] uppercase opacity-60">
            Selection
          </span>
        </div>
          ...
          <div ref={scrollRef} className="flex gap-3 w-max py-2">
            {[...displayMovies, ...displayMovies].map((movie, index) => (
              ...
              >
                ...
                {/* 装饰性背景文字：缩小字号 */}
                <span className="absolute top-1 right-2 text-3xl font-black text-text-main/20 select-none z-0">
                  {movie.year.slice(-2)}
                </span>
                ...
                <div className="relative z-10 p-3 flex flex-col justify-between h-full">
                  <div>
                    ...
                    <h3 className="text-[11px] font-bold text-text-main leading-tight tracking-tight uppercase wrap-break-word drop-shadow-lg">
                      {movie.title}
                    </h3>
                  </div>

                  <div>
                    <p className="text-[9px] text-text-main/80 font-mono tracking-widest uppercase drop-shadow-md">
                      {movie.year}
                    </p>
                  </div>
                </div>
